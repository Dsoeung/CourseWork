...
int readRec(HashFile *pHashFile, int iRBN, void *pRecord)
{
	...
	fread(&bTemp ,pHashFile->hashHeader.iRecSize, 1L, pHashFile->pFile);
    *(Book*)pRecord = bTemp;
	
    if(fread(&bTemp ,pHashFile->hashHeader.iRecSize, 1L, pHashFile->pFile) != 1 || bTemp.szBookId[0] == '\0'){
...
int updateBook(HashFile *pHashFile, Book *pBook)
{
    ...
    for(k=0;k<pHashFile->hashHeader.iMaxProbes;k++)
    {
        if(iRBN+k > pHashFile->hashHeader.iMaxPrimary)
        {
            return RC_REC_NOT_FOUND; // went past max amount of records
    ...
    // if loop ends max probes reached
	//printf("Update loop ends \n");
    return RC_REC_NOT_FOUND;
}

/******************** deleteBook **********************************
    int deleteBook(HashFile *pHashFile, char *pszBookId)
Purpose:
    This function finds the specified book and deletes it by simply
    setting all bytes in that record to '\0'.
Parameters:
    I HashFile *pHashFile       contains HashHeader and File pointer
    I char *pszBookId        Book id
Return value:
**************************************************************************/
