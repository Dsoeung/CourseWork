...
int insertBook(HashFile *pHashFile, Book *pBook){
    ...
    int rec_num = readRec(pHashFile, iRBN, &bTemp);
    ...
    for(k=1;k<pHashFile->hashHeader.iMaxProbes;k++){
		rc = readRec(&(*pHashFile), RBN+i, &bTemp);
			//if empty slot
            ...
            }else if(rc == RC_OK && strcmp(pBook->szBookId, bTemp.szBookId)==0)
            {
                return RC_REC_EXISTS;
            }
	}
...
int updateBook(HashFile *pHashFile, Book *pBook)
{
    ...
    for(k=0;k<pHashFile->hashHeader.iMaxProbe;k++)
    {
        if(iRBN+k > pHashFile->hashHeader.iMaxPrimary)
        {
            return RC_REC_NOT_FOUND; // went past max amount of records
...
Return value:
**************************************************************************/
int deleteBook(HashFile *pHashFile, char *pszVehicleId)
{
