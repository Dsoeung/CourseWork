...
int insertBook(HashFile *pHashFile, Book *pBook){
	...
	//IF that location doesn't exist (== NULL) or (||) the record at that location has a szBookId[0] == '\0':
	...
	else
	{
		...
		if(strncmp( pBook -> szBookId, bTemp.szBookId, sizeof(bTemp.szBookId))== 0)
		{
			...
			//If that record exists and that book's szBookId does not match pBook->szBookId
		//Otherwise, return RC_SYNONYM.
		//Synonyms will be prevalent in Assignment 2
		return RC_SYNONYM;
	}
	//Making sure all branches have a return so returning RC_OK
	return RC_OK;
}
