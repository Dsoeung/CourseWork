...
Input:  
    ...
    - Customer Identification Information:
        ...
        o Although szFullName is a maximum of 30 characters, it may 
          ...
          code: %30[^n] which doesn't have an 's' in it.
...
printf("XXXD");
    fclose(pFileCust);
    printf("Done");    // included so that you can put a breakpoint on this line
    return 0;
}
...
{
    FILE *pFileCust = fopen("p1customer.txt", "r");
    int inputCount;
    double balance;
    char line[MAX_LINE_SIZE];//max 100
    char id[7], email[41], name[31];
    char streetaddress[31], city[21], state[3], zip[6];
    char transtype[2], bookid[10], transdt[11];


    while(fgets(line, MAX_LINE_SIZE, pFileCust) !=NULL) //Loop
    {
        printf("******************** Library Receipt ********************\n");
        inputCount = sscanf(line, "%6s %lf %40s %30[^\n]\n", id, &balance, email, name);
        printf(line, "%6s %lf %40s %30[^\n]\n", id, &balance, email, name);
        if (inputCount != 4)
            


(fgets(line, MAX_LINE_SIZE, pFileCust) !=NULL);

inputCount = sscanf(line, "%30[^,] %20s %2s %5s\n", streetaddress, city, state, zip);
printf(line, "%30[^,] %20s %2s %5s\n", streetaddress, city, state, zip);
while(fgets(line, MAX_LINE_SIZE, pFileCust) !=NULL)
{
    inputCount = sscanf(line, "%2c %9s %10s\n", transtype, bookid, transdt);
    printf(line, "trans = %2c Book = %9s Date = %10s\n", transtype, bookid, transdt);
    if (transtype == "X")
        break;
        }        
    }
...
{
    if (iArg >= 0)
        fprintf(stderr, "Error: bad argument #%d.  %s %sn"
            , iArg
            , pszMessage
            , pszDiagnosticInfo);
    fprintf(stderr, "p1 -c customerTransactionFilen");
    if (iArg >= 0)
        exit(-1);
    else
        exit(-2);
