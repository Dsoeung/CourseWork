...
void printBooks(char *pszHeading, NodeT *pRoot)
{
    ...
    // output each book using PrintInOrder recursive function. (must make).
	printInOrder(pRoot);
    
}

//Recursive function for printing out the books.
...
void printInOrder (NodeT* p)
{
	...
	printf("   %-9s %-40s %-8s %-10s %8.2lf %8.2lf \n"
            , p->book.szBookId				//bookM[i].szBookId
            , p->book.szTitle				//bookM[i].szTitle
            , p->book.szCustomerId			//bookM[i].szCustomerId
            , p->book.szCheckedOutDt		//bookM[i].szCheckedOutDt
            , p->book.dLateFeePerDay		//bookM[i].dLateFeePerDay
            , p->book.dMaxLateFee			//bookM[i].dMaxLateFee
            );
                                    ...
                                    char *pszRemainingInput, Customer *pCustomer)
{
   ...
   // printf("%s", pszSubCommand);
    ...
    else if (strcmp(pszSubCommand, "TRANS") == 0)
    {
        ...
        pCustomer->dFeeBalance += processTransaction(pRoot, *pCustomer, transaction);
...
{
	Book book;
	NodeT* p = searchT(*ppRoot, book.szBookId);		
    int iScanfCnt;
    int i;
    // Determine what to do based on the subCommand
    // your code
	...
	if(strcmp(pszSubCommand, "BOOK PPRINT") == 0){
			//print binary tree in prettyprint
...
double processTransaction(NodeT* pRoot, Customer customer, Transaction transaction){
    NodeT* i =  searchT(pRoot, transaction.szBookId);
    double lateFee;
    //If cannot find book searchT will return NULL. (Make SearchT)
        if(i == NULL)
...
NodeT *searchT(NodeT *p, char szMatchBookId[]){
	if(p==NULL)
		return NULL;
	if(szMatchBookId == p->book.szBookId)
		return p;
	if(szMatchBookId < p->book.szBookId)
...
NodeT *insertT(NodeT *p, Book book){
	...
	if(book.szBookId == p->book.szBookId)
		return p;
	else if (book.szBookId < p->book.szBookId){
...
int i;
	if (p == NULL)
		return;
	prettyPrintT(p->pRight, iIndent+3);
	for (i = 0; i < iIndent; i++) printf(" ");
	printf("%s\n", p->book.szBookId);
	prettyPrintT(p->pLeft, iIndent+3);
}
