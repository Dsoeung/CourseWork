    ...
    if (pFileBook == NULL)
        exitError(ERR_BOOK_FILENAME, pszBookFileName);
             ...
             while (fgets(szInputBuffer, 100, pFileBook) != NULL)
             {
                 ...
                 // It should have populated all six variables, if not exit with an error.
                 if (iScanfCnt < 6)
...
Purpose:Sorting the books from the Text file using bubble sort to put them in the correct order.

Parameters: 1: Book bookM[]    an array which is used for returning the books.
            2: int iBookCnt    used to see how many books are being passed through bookM[] along with how many times it will go through the loops
...
**************************************************************************/
void processCustomerCommand(Book bookM[], int iBookCnt, char *pszSubCommand,
                                    char *pszRemainingInput, Customer *pCustomer)
{
   ...
   // printf("%s", pszSubCommand);
    ...
    else if (strcmp(pszSubCommand, "TRANS") == 0)
    {
        ...
        pCustomer->dFeeBalance += processTransaction(bookM, iBookCnt, *pCustomer, transaction);
...
**************************************************************************/
void processBookCommand(Book bookM[], int iBookCnt, char *pszSubCommand, char *pszRemainingInput)
{
    Book book;
    int iScanfCnt;
    int i;
    // Determine what to do based on the subCommand
    // your code
    iScanfCnt = sscanf(pszRemainingInput, "%9s %lf %lf", book.szBookId,
    ...
    if(strcmp(pszSubCommand, "CHANGE") == 0)
    {
        ...
        if(i > -1)
        {
            ...
            printf(" %-9s %-40s %-8s %-10s %-8.2lf %-8.2lf \n",  bookM[i].szBookId, bookM[i].szTitle, bookM[i].szCustomerId,
            bookM[i].szCheckedOutDt,bookM[i].dLateFeePerDay, bookM[i].dMaxLateFee);
        }
   }
...
double processTransaction(Book bookM[], int iBookCount, Customer customer, Transaction transaction){
    int i =  searchBooks(bookM, iBookCount, transaction.szBookId);
    double lateFee;
    //The -1 comes from searchBooks  to return book not found if it is not avaliable 
        if(i == -1)
    ...
    //based on the character 'C' or 'R' the program would follow the right cutsomer that is read
        ...
        {       
          ...
          {
              sscanf(customer.szCustomerId, "%7s", bookM[i].szBookId); 
              sscanf(transaction.szTransDt, "%11s", bookM[i].szCheckedOutDt);
          }
        }  
        if(transaction.cTransType == 'R')
        {
            if(dateDiff(transaction.szTransDt, (bookM[i].szCheckedOutDt)) > 14)
            {
               // printf("%s", bookM[i].szCheckedOutDt);         
                int difference = dateDiff(transaction.szTransDt, bookM[i].szCheckedOutDt);
               //printf("&s", bookm[i].szCheckedOutDt);
                int numDays = (difference - 14);
                 lateFee = (numDays * bookM[i].dLateFeePerDay);

                if(lateFee > bookM[i].dMaxLateFee)
                {
                   lateFee = bookM[i].dMaxLateFee;
                  customer.dFeeBalance = (customer.dFeeBalance + lateFee);  
                }
                    printf("\tLate Fee = %.2lf", lateFee);
            } 
                sscanf("NONE", "%7s", bookM[i].szCustomerId);
                sscanf("0000-00-00", "%11s", bookM[i].szCheckedOutDt);
        }                   
         }
